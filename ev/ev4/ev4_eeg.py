...
csv_path = f"E:/data6/ev/ev4/Data/ev4p{pid}s{sid}.csv"
...
#square setup
Pixel = mon.getSizePix()[0] / mon.getWidth()
X = win.size[0] / 2 / Pixel - 1
Y = win.size[1] / 2 / Pixel - 1
SqaureFrame = 3
TrialSquare = visual.Rect(win, width=2, height=2, color=(1, 1, 1), pos=(X, Y))
HeadSquare = visual.Rect(win, width=2, height=2, color=(1, 1, 1), pos=(-X, Y))
TailSquare = visual.Rect(win, width=2, height=2, color=(1, 1, 1), pos=(-X, -Y))
...
def trial(trialCount, onFrames, pHead):
    ...
    while frame < trialFrames:
        ...
        if frame < SqaureFrame:
            TrialSquare.draw()
            if trialData["isHead"] == 1: HeadSquare.draw()
            elif trialData["isHead"] == 0: TailSquare.draw()
